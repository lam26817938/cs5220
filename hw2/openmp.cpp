...
vector<vector<omp_lock_t>> gridLocks;
//int NUM_THREADS=16;
...
void init_simulation(particle_t* parts, int num_parts, double size) {
   ...
   // omp_set_num_threads(NUM_THREADS);
    ...
    gridLocks.resize(gridSize, vector<omp_lock_t>(gridSize));
...
#pragma omp for
    for (int i = 0; i < num_parts; ++i) {
        ...
        if (x_old != x_new || y_old != y_new) {
            omp_set_lock(&gridLocks[x_old][y_old]);
            auto& oldCell = grid[x_old][y_old];

            oldCell.erase(remove(oldCell.begin(), oldCell.end(), i), oldCell.end());
            omp_unset_lock(&gridLocks[x_old][y_old]);

            omp_set_lock(&gridLocks[x_new][y_new]);
            grid[x_new][y_new].push_back(i);
            omp_unset_lock(&gridLocks[x_new][y_new]);
        }
    }
